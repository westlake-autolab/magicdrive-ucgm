...
from magicdrivedit.schedulers.scheduler import BaseScheduler
...
def mean_flat(tensor: torch.Tensor, mask=None, dim=None):
    """
...
def timestep_transform(
    ...
    for key in ["height", "width", "num_frames"]:
        if model_kwargs[key].dtype in (torch.float16, torch.bfloat16):
...
class RFlowScheduler(BaseScheduler):
    def __init__(
        self,
        num_timesteps=1000,
        ...
        **kwargs
        ...
        super(RFlowScheduler, self).__init__(
            num_timesteps=num_timesteps, num_sampling_steps=num_sampling_steps,
            num_infer_sampling_steps=num_infer_sampling_steps,
            use_timestep_transform=use_timestep_transform, transform_scale=transform_scale,
            **kwargs)
    ...
    def prepare_sampled_timesteps(self, B, device, additional_args=None, num_timesteps=None, num_sampling_steps=None, with_zero=False):
        if num_timesteps is None:
            num_timesteps = self.num_timesteps
        if num_sampling_steps is None:
            num_sampling_steps = self.num_sampling_steps
        if with_zero:
            timesteps = [(1.0 - i / num_sampling_steps) * num_timesteps for i in range(num_sampling_steps + 1)]
        else:
            timesteps = [(1.0 - i / num_sampling_steps) * num_timesteps for i in range(num_sampling_steps)]
        print(timesteps)
        if self.use_discrete_timesteps:
            timesteps = [int(round(t)) for t in timesteps]
        timesteps = torch.stack([torch.tensor([t] * B, device=device, dtype=torch.float32) for t in timesteps], dim=0)
        print(timesteps)
        if self.use_timestep_transform:
            timesteps = torch.stack([timestep_transform(
                t, additional_args, num_timesteps=num_timesteps,
                cog_style=self.cog_style_trans,
            ) for t in timesteps], dim=0)
        print(timesteps)
        return timesteps
    ...
    def training_losses(self, model, x_start, model_kwargs=None, noise=None, mask=None, weights=None, t=None):
        ...
        if weights is None:
            loss = mean_flat((velocity_pred - (noise - x_start)).pow(2), mask=mask)
        else:
            weight = _extract_into_tensor(weights, t, x_start.shape)
            loss = mean_flat(weight * (velocity_pred - (noise - x_start)).pow(2), mask=mask)

        total_loss = loss

        if torch.any(loss > 5 | loss.isnan()):
            print(loss)
        # if torch.any(loss.isnan()):
        #     print(f'x_start: {x_start}, model_output: {model_output}, velocity_pred: {velocity_pred}, mask: {mask}')
        terms["loss"] = loss
