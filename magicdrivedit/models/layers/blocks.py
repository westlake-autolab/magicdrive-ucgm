...
from magicdrivedit.acceleration.communications import all_to_all, split_forward_gather_backward
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        caller_location = f"{caller_info.filename.split('/')[-1]}:{caller_info.lineno}"
        logging.debug(f"🔍 [{layer_name}@{caller_location}] INPUT: x.shape={x.shape}, N={N}, B={B}, enable_flash_attn={enable_flash_attn}, self.enable_flash_attn={self.enable_flash_attn}")
        qkv = self.qkv(x)
        ...
        if enable_flash_attn:
            ...
            if USE_NPU:
                ...
                # Flash attention executed
                logging.debug("🔥 FLASH ATTENTION EXECUTED!")
        elif self.enable_xformers:
            if verbose:
                logging.debug(f"[{self.__class__}] use xformers")
            attn_bias = None
            x = xformers.ops.memory_efficient_attention(q, k, v, p=self.attn_drop.p, attn_bias=attn_bias)
            # Xformers attention executed
            logging.debug("⚡ XFORMERS ATTENTION EXECUTED!")
        else:
            if verbose:
            ...
            # Torch attention executed
            logging.debug("🐌 TORCH ATTENTION EXECUTED!")
        ...
        caller_info = inspect.stack()[1]
        caller_location = f"{caller_info.filename.split('/')[-1]}:{caller_info.lineno}"
        logging.debug(f"🔍 [{layer_name}@{caller_location}] OUTPUT: x.shape={x.shape}")
        return x
...
class SeqParallelAttention(Attention):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        caller_info = inspect.stack()[1]
        caller_location = f"{caller_info.filename.split('/')[-1]}:{caller_info.lineno}"
        logging.debug(f"🔍 [{layer_name}@{caller_location}] INPUT: x.shape={x.shape}, N={N}, B={B}")
        qkv = self.qkv(x)
        qkv_shape = (B, N, 3, self.num_heads, self.head_dim)
        ...
        if self.enable_flash_attn:
            if USE_NPU:
                ...
                # Seq parallel flash attention executed
                logging.debug("🔥 SEQ PARALLEL FLASH ATTENTION EXECUTED!")
        else:
            dtype = q.dtype
            q = q * self.scale
            attn = q @ k.transpose(-2, -1)  # translate attn to float32
            ...
            attn = self.attn_drop(attn)
            x = attn @ v
            # Seq parallel torch attention executed
            logging.debug("🐌 SEQ PARALLEL TORCH ATTENTION EXECUTED!")
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cond: torch.Tensor = None) -> torch.Tensor:
        ...
        caller_info = inspect.stack()[1]
        caller_location = f"{caller_info.filename.split('/')[-1]}:{caller_info.lineno}"
        logging.debug(f"🔍 [{layer_name}@{caller_location}] INPUT: x.shape={x.shape}, cond.shape={cond.shape if cond is not None else 'None'}, N={N}, B={B}, enable_flash_attn={enable_flash_attn}")
...
class SeqParallelMultiHeadAttention(MultiHeadAttention):
    ...
    def forward(self, x: torch.Tensor, cond: torch.Tensor = None) -> torch.Tensor:
        ...
        caller_info = inspect.stack()[1]
        caller_location = f"{caller_info.filename.split('/')[-1]}:{caller_info.lineno}"
        logging.debug(f"🔍 [{layer_name}@{caller_location}] INPUT: x.shape={x.shape}, cond.shape={cond.shape if cond is not None else 'None'}, N={N}, B={B}")
...
class SeqParallelMultiHeadCrossAttention(MultiHeadCrossAttention):
    ...
    def forward(self, x, cond, mask=None):
        ...
        caller_info = inspect.stack()[1]
        caller_location = f"{caller_info.filename.split('/')[-1]}:{caller_info.lineno}"
        logging.debug(f"🔍 [{layer_name}@{caller_location}] INPUT: x.shape={x.shape}, cond.shape={cond.shape if cond is not None else 'None'}, mask={mask is not None}")
        ...
        if USE_XFORMERS:
            if USE_NPU:
                ...
                logging.debug("🔥 SEQ PARALLEL NPU FUSION ATTENTION EXECUTED!")
                x = x.transpose(1, 2).contiguous()
            else:
                ...
                x = xformers.ops.memory_efficient_attention(
                    ...
                    # NOTE: head_num is not real, we should set scale!
                    scale=self.scale)
                logging.debug("⚡ SEQ PARALLEL XFORMERS ATTENTION EXECUTED!")
        else:
