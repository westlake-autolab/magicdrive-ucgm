...
scheduler = dict(
    type="ucgm",
    transport_type="ReLinear",  # 运输类型：ReLinear, Linear, TrigFlow, TrigLinear
    consistc_ratio=0.0,  # 0.0=多步模型, 1.0=少步模型, 0.0-1.0=混合训练
    enhanced_target_config=dict(
        enhanced_use_ema=True,      # 使用 EMA 模型生成增强目标
        enhanced_ratio=0.5,         # 增强目标比例 (0.0-1.0)
        enhanced_style="fc-vs-fe",        # 增强模式: "fc-vs-fe", "ft-vs-fe", "fc-vs-xz"
        enhanced_range=[0.00, 0.75], # 增强时间步范围 - 与ucgm一致
    ),
    loss_config=dict(
        scaled_cbl_eps=0.0,         # Pseudo Huber 损失的 epsilon 参数
        wt_cosine_loss=False,       # 是否使用余弦损失
        weight_function=None,       # 权重函数: "Cosine" 或 None
        dispersive_loss_weight=None,# 分散性损失权重
        mean_var_loss_weight=None,  # 均值方差损失权重
    ),
    time_dist_ctrl=[1.0, 1.0, 1.0],  # Beta 分布参数 [alpha, beta, scale]
    infer_config=dict(
        time_dist_ctrl=[1.0, 1.0, 1.0],  # 推理时的时间分布控制
        stochast_ratio=0.0,              # 随机性比例
        extrapol_ratio=0.0,              # 外推加速比例
        sampling_order=1,                # 采样阶数 (1 或 2)
        rfba_gap_steps=[0.001, 0.001],   # 边界间隔步长
    ),
