...
# ===== 内存监控工具导入 =====
try:
    from memory_monitor import log_gpu_memory, bytes2human
    MEMORY_DEBUG = True
    print("✅ Memory monitoring enabled")
except ImportError:
    MEMORY_DEBUG = False
    print("❌ Memory monitoring disabled (memory_monitor.py not found)")
    def log_gpu_memory(*args, **kwargs):
        pass
    def bytes2human(n):
        return f"{n} bytes"
...
def main():
    ...
    # ===== 内存调试配置 =====
    MEMORY_DEBUG_EARLY_STOP = int(os.environ.get("MEMORY_DEBUG_STEPS", "3"))  # 通过环境变量控制调试步数
    ...
    if MEMORY_DEBUG:
        print(f"\n{'='*80}")
        print(f"🔍 MEMORY DEBUG MODE - Rank {dist.get_rank()}")
        print(f"Will stop after {MEMORY_DEBUG_EARLY_STOP} steps")
        print(f"{'='*80}")
        log_gpu_memory("init", rank=dist.get_rank())
    ...
    log_gpu_memory("before_dataset", rank=dist.get_rank())
    ...
    log_gpu_memory("after_dataset", rank=dist.get_rank())
    ...
    log_gpu_memory("after_dataloader", rank=dist.get_rank())
    ...
    log_gpu_memory("after_text_encoder", rank=dist.get_rank())
    ...
    log_gpu_memory("after_vae", rank=dist.get_rank())
    ...
    log_gpu_memory("after_main_model", rank=dist.get_rank())
    ...
    log_gpu_memory("after_ema", rank=dist.get_rank())
    ...
    log_gpu_memory("after_optimizer", rank=dist.get_rank())
    ...
    log_gpu_memory("before_boost", rank=dist.get_rank())
    ...
    log_gpu_memory("after_boost", rank=dist.get_rank())
    ...
    log_gpu_memory("training_start", rank=dist.get_rank())
    ...
    for epoch in range(start_epoch, cfg_epochs):
        ...
        with tqdm(
            ...
            for step, batch in pbar:
                ...
                # ===== 内存调试步骤开始 =====
                if MEMORY_DEBUG and coordinator.is_master():
                    print(f"\n{'='*100}")
                    print(f"🔍 STEP {step} MEMORY ANALYSIS - Rank {dist.get_rank()}")
                    print(f"{'='*100}")
                
                log_gpu_memory(f"step_{step}_start", rank=dist.get_rank())
                ...
                log_gpu_memory(f"step_{step}_data_moved", rank=dist.get_rank())
                ...
                log_gpu_memory(f"step_{step}_encoded", rank=dist.get_rank())
                ...
                log_gpu_memory(f"step_{step}_args_ready", rank=dist.get_rank())
                ...
                log_gpu_memory(f"step_{step}_before_forward", rank=dist.get_rank())
                
                # == diffusion loss computation ==
                nulls = {}
                if 'y' in model_args:
                  y_feat = model_args['y']
                  if hasattr(text_encoder, 'y_embedder') and getattr(text_encoder, 'y_embedder', None) is not None:
                      # 用模型自己的 null()，并对齐 y 的 device/dtype
                      nulls['y'] = text_encoder.null(y_feat.shape[0]).to(y_feat.device, y_feat.dtype)
                  else:
                        B  = y_feat.shape[0]
                        Ly = y_feat.shape[-2] if y_feat.ndim >= 3 else 1
                        D  = y_feat.shape[-1]
                        nulls['y'] = torch.zeros(B, 1, Ly, D, device=y_feat.device, dtype=y_feat.dtype)
            
                loss_dict = scheduler.ddm_training_losses(
                    model=model,
                    teacher_model=None,
                    ema_model=ema,
                    x_start=x,
                ...
                log_gpu_memory(f"step_{step}_after_forward", rank=dist.get_rank())
                ...
                log_gpu_memory(f"step_{step}_before_backward", rank=dist.get_rank())
                ...
                log_gpu_memory(f"step_{step}_after_backward", rank=dist.get_rank())

                if verbose_mode:
                    logger.info(f"Start after step ops! step={step}")
                # == update EMA ==
                with timers["update_ema"] as ema_t:
                    update_ema(ema, model.module, optimizer=optimizer, decay=cfg.get("ema_decay", 0.9999), sharded=False)
                if record_time:
                    timer_list.append(ema_t)

                log_gpu_memory(f"step_{step}_after_ema", rank=dist.get_rank())
                ...
                # ===== 内存调试步骤总结 =====
                if MEMORY_DEBUG and coordinator.is_master():
                    print(f"\n📊 Step {step} Memory Summary:")
                    torch.cuda.synchronize()
                    current_alloc = torch.cuda.memory_allocated()
                    max_alloc = torch.cuda.max_memory_allocated()
                    print(f"   Current: {bytes2human(current_alloc)}")
                    print(f"   Peak:    {bytes2human(max_alloc)}")
                    print(f"{'='*100}\n")
                ...
                # ===== 内存调试早期停止 =====
                if MEMORY_DEBUG and step >= MEMORY_DEBUG_EARLY_STOP - 1:
                    if coordinator.is_master():
                        print(f"\n🔍 Memory debug completed after {step+1} steps")
                        print(f"Check the logs above for memory usage analysis")
                    break
                    
                if enable_debug and step > 50:
                    break
        if enable_debug or (MEMORY_DEBUG and step >= MEMORY_DEBUG_EARLY_STOP - 1):
            break
        sampler.reset()
        start_step = 0
